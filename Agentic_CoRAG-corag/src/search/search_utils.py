...
def build_search_queries(query: str, max_variants: int = 4) -> List[str]:
    ...
    patterns = [
        ...
        (r"^Who composed (?:the music for )?(?:(?:the|a) )?(?:film|song|movie|album|show|play|series) (?P<title>.+?)\?$", lambda m: [f"{m.group('title')} composer", f"composer {m.group('title')}", f"music by {m.group('title')}"]),
