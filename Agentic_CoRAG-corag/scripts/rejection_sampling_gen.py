...
def main():
    ...
    parser.add_argument("--vllm_api_base", type=str, default="http://localhost:8083/v1", help="vLLM OpenAI API base URL, e.g. http://localhost:8000/v1")
    ...
    parser.add_argument("--tokenizer_path", type=str, default="", help="Local tokenizer/model path on THIS machine (used for AutoTokenizer). ""Useful when --model is a remote vLLM model path.")
    ...
    parser.add_argument("--max_message_length", type=int, default=4096*2, help="Max message length for truncation (0 or negative to disable truncation)")
    ...
    # If using HF json loader with a local json/jsonl file path, use data_files instead of config name
    if args.dataset == "json" and args.subset and os.path.exists(args.subset):
        ds_dict = load_dataset("json", data_files=args.subset)
        # json loader returns a DatasetDict with "train" split by default
        ds = ds_dict[args.split] if args.split in ds_dict else ds_dict["train"]
    else:
        ds = load_dataset(args.dataset, args.subset, split=args.split)
    ...
    if getattr(args, "tokenizer_path", ""):
        tokenizer = AutoTokenizer.from_pretrained(
            args.tokenizer_path, use_fast=True, local_files_only=True
        )
        print(f"[info] Loaded local tokenizer from: {args.tokenizer_path}")
    else:
        # Strongly recommended when vLLM model id is a remote filesystem path
        print("[warn] --tokenizer_path not provided. If vLLM model id is a remote path (e.g. /home/models/llm/...), "
              "tokenizer auto-init may fail. Recommended: --tokenizer_path /home/models/Qwen3-8B")

    base_agent = CoRagAgent(vllm_client=vllm, corpus=dummy_corpus, graph_api_url=args.graph_api_url, tokenizer=tokenizer)
