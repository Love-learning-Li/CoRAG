...
### 1.0 准备 QA 数据集对应的 corpus 语料库

在运行数据集构造流水线之前，需要准备一个 corpus 语料库。corpus 是一个 JSONL 文件，每行是一个文档对象，包含以下字段：
...
```json
{"title": "Havana", "text": "The current mayor of Havana (\"President of the People's Power Provincial Assembly\") is Marta Hernández Romero, she was elected on March 5, 2011."}
{"title": "Isaac James", "text": "Isaac James is an American rock band, with roots based in both Kansas City, Missouri and Des Moines, Iowa, United States."}
```

#### 将 corpus 构建为 chunk 映射与 FAISS 索引（用于朴素检索 / 本地向量检索）

使用脚本 `naive_rag_retrieve/build_misique_index.py` 将上述 corpus：

- 切分为多个 **chunks**（文本片段）
- 计算每个 chunk 的 embedding
- 构建并保存 **FAISS** 向量索引
- 同时保存 **chunk_id -> chunk_text** 的映射文件（JSONL）

**输入格式**：JSONL，每行 `{"title": "...", "text": "..."}`  
**输出文件**（脚本当前默认写死在代码里，可按需修改）：

- `naive_rag_retrieve/musique_faiss.index`：FAISS 索引文件
- `naive_rag_retrieve/musique_chunks.jsonl`：chunk 映射文件（每行 `{"id": 0, "text": "..."}`）

运行方式（Linux）：

```bash
python naive_rag_retrieve/build_misique_index.py
```

脚本关键参数（在 `build_misique_index.py` 内修改）：

- `CORPUS_PATH`：你的 corpus 路径（JSONL）
- `INDEX_PATH` / `CHUNKS_PATH`：索引与 chunks 输出路径
- `CHUNK_SIZE` / `CHUNK_OVERLAP`：切分粒度（默认 1000/100）
- `EMB_MODEL_NAME`：embedding 模型（默认 `BAAI/bge-large-en-v1.5`）
- `EMB_DEVICE`：运行设备（如 `cuda:0` / `cuda:1` / `cpu`）
- `MAX_DOCS_FOR_DEBUG`：调试用，限制处理文档数


#### 启动本地 MuSiQue FAISS 检索 API 服务（FastAPI）

当你已经生成好：

- `naive_rag_retrieve/musique_faiss.index`
- `naive_rag_retrieve/musique_chunks.jsonl`

即可使用 `naive_rag_retrieve/retrieve_app.py` 启动一个简单的 HTTP 检索服务，用于后续数据构造/训练/评测时调用

1) 启动服务（Linux）：

```bash
cd naive_rag_retrieve

# 建议：禁用用户级 site-packages，避免 numpy/faiss 版本冲突
PYTHONNOUSERSITE=1 python retrieve_app.py
```

默认监听地址与端口：

- `http://0.0.0.0:9600`（对外可通过服务器实际 IP 访问，如 `http://10.70.113.80:9600`）
...
- **请求体**：
  ...
  - `top_k`（int，可选，默认 5）
- **响应体**：
  - `chunks`（string 列表）：返回的 top-k 文本片段

3) curl 测试示例：

在任意可访问服务器的机器上运行（把 IP 改成你的服务器 IP）：

```bash
curl -s http://10.70.113.80:9600/search \
...
python scripts/rejection_sampling_gen.py \
  ...
  --tokenizer_path /home/models/Qwen3-8B \
  --n_samples 5 \
  --temperature 0.7 \
  --max_path_length 3
```
